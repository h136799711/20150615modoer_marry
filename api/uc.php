...
class uc_note {
	...
	function deleteuser($get, $post) {
        ...
        $this->db->exec("DELETE FROM {$this->dbpre}member_address WHERE uid IN ($uids)");
        $this->db->exec("DELETE FROM {$this->dbpre}member_passport WHERE uid IN ($uids)");
        $this->db->exec("DELETE FROM {$this->dbpre}member_point WHERE uid IN ($uids)");
        $this->db->exec("DELETE FROM {$this->dbpre}member_profile WHERE uid IN ($uids)");
	...
	function synlogin($get, $post) {
        ...
        $cookietime = 31536000;
        $q = $this->db->query("SELECT uid,username,password FROM {$this->dbpre}members WHERE uid='$uid'");
        if($q) {
        ...
        if($member) {
	        ...
	        $hash = mo_formhash($uid, $username, $member['password']);
	        $str = $uid . "\t" . md5($hash);
            mo_setcookie('hash', mo_authcode($str,'ENCODE'), $cookietime);
        } else {
	...
	function updatecredit($get, $post) {
        ...
        if($q) {
            ...
            if(isset($r[$point])) {
                ...
                $this->db->exec("UPDATE {$this->dbpre}member_point SET $point=$point+$amount WHERE uid=$uid");
                //兑换记录
                $this->db->exec("INSERT INTO {$this->dbpre}member_point_log(in_uid,in_username,in_point,in_value,dateline,des) 
                    VALUES($uid,'$r[username]','$point','$amount',$time,'UCenter用户积分兑换')");
            }
        } else {
            return API_RETURN_FORBIDDEN;
        }
...
/**************** Modoer库函数 ********************/
//简单加密
function mo_authcode($string, $operation = 'DECODE') {
