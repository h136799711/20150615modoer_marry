...
function item_style_init_operation () {
    var op_banner = $('<a href="javascript:;">更换横幅</a>').attr('id','item-op-banner')
        .click(function() {item_change_banner(sid)});

    var op_bcastr = $('<a href="javascript:;">管理橱窗</a>').attr('id','item-op-bcastr')
        .click(function() {item_manage_bcastr(sid)});

    item_style_op('item-manage-op-banner','item_banner', op_banner);
    item_style_op('item-manage-op-bcastr','item-index-bcastr-box', op_bcastr);
}

function item_style_op(myid,boxid,op) {
    var operation = $('#' + myid);
    if(!operation[0]) {
        operation = $('<div></div>').attr('id',myid).addClass('item-manage').hide().append(op);
    }

    $('#'+boxid).append(operation)
        .mouseover(function() {
            operation.show();
        })
        .mouseout(function() {
    ...
    var left = $('#'+boxid).offset().left + $('#'+boxid).width() - width - 10;
