...
<div class="sub-menu">
    ...
    <a href="<?=cpurl($module,$act)?>" class="sub-menu-item">菜单列表</a>
    <a href="<?=cpurl($module,$act,'add')?>" class="sub-menu-item<?if($op=='add'):?> selected<?endif;?>">添加菜单</a>
    <?if($op=='edit'):?>
    <a href="#" class="sub-menu-item selected">编辑菜单</a>
    <?endif;?>
</div>
    ...
    <div class="space">
        <table class="maintable" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr id="tr_url"<?if($menu['isfolder']):?>style="display:none;"<?endif;?>>
                ...
                <td>
                    <input type="text" name="menu[url]" id="menu_url" class="txtbox2" value="<?=$menu['url']?>" />
                    <select onchange="$('#menu_url').val(this.value);">
                    <option value="">=自动填写=</option>
                        <option value="modoer/index">首页</option>
            ...
            <tr>
                ...
                <td><input type="text" name="menu[scriptnav]" class="txtbox2" value="<?=$menu['scriptnav']?>" /></td>
            </tr>
            <tr>
                <td class="altbg1" width="45%"><strong>菜单图标:</strong>如果使用到带图标的菜单项时提供图标地址，默认图标存放在 ./static/images/menu 下</td>
                <td><input type="text" name="menu[icon]" class="txtbox2" value="<?=$menu['icon']?>" /></td>
            </tr>
            <tr id="tr_target"<?if($menu['isfolder']):?>style="display:none;"<?endif;?>>
                <td class="altbg1" width="45%"><strong>目标框架:</strong>可以设置页面打开到目标位置</td>
                <td><input type="text" name="menu[target]" class="txtbox2" value="<?=$menu['target']?>" />&nbsp;例如: _blank, _self</td>
            </tr>
            <tr>
