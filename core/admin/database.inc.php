			...
			if($rows = $db->query("SELECT * FROM $table LIMIT $startfrom, $offset")) {
                ...
                while($row = $rows->fetch_row()) {
                    ...
                    for($i = 0; $i < $numfields; $i++) {
                        $tabledump .= $comma.'\''.$db->_escape_str($row[$i]).'\'';
                        $comma = ',';
                    }
                    $tabledump .= ");\n";
                }
...
function database_function() {
    ...
    while ($v=$q->fetch_array()) {
        if($db==$v['Db']) $funcs[] = $v['Name'];
    }
    $q->free_result();
    $result = '';

    foreach ($funcs as $fun_name) {
        $q = $_G['db']->query("select * from mysql.proc where name='$fun_name'");
        if(!$q) continue;
        $detail = $q->fetch_array();
        $createsql = "DROP FUNCTION IF EXISTS `{$detail['name']}`;\n";
        $createsql .= "DELIMITER ;;;\n";
        $createsql .= "CREATE FUNCTION `{$detail['name']}`({$detail['param_list']})\n";
        $createsql .= trim("RETURNS {$detail['returns']} " . str_replace("_", " ", $detail['sql_data_access'])) . "\n";
        $createsql .= $detail['body'] . ";;;\n";
        $result .= $createsql;
    }
    return $result;
}
