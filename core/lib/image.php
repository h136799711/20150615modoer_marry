...
class ms_image {
	...
	var $thumb_mod              = 0;                //缩略图方式，0表示按比例裁剪,1为等比例缩放
	var $thumb_level            = 80;               //缩略图缩小质量(80~100)
	...
	//调整图片的尺寸
	function auto_resize($img, $max_width, $max_height) {
		$max_width = abs((int) $max_width);
		$max_height = abs((int) $max_height);
		if(!$max_height || !$max_height) return;
		if(!is_file($img)) return;
		list($width, $height, $type, $attr) = @getimagesize($img);
		if($width>$max_width['width'] || $height>$max_height['height']) {
			$this->thumb($img, $img, $max_width, $max_height);
		}
	}

	//截取缩略图
	function thumb($source_img_file, $dest_img_file, $new_width, $new_height, $level = 0) {
		$thumbfunc = $this->thumb_mod ? '_thumb2' : '_thumb';
		if(!$level || $level < 0) $level = $this->thumb_level;
	...
	function _thumb($source_img_file, $dest_img_file, $new_width, $new_height, $level = 80) {
		...
		if ($img) {
			...
			redirect(lang('global_upload_image_dnot_support')."($source_img_file)");
		}
	}
	...
	*/
	...
	function _imagecreatefromimg($img) {
		...
		switch($ext_name) {
			...
			case 'jpe':
			case 'jpeg':
				//jpg文件可能商城句柄失败，则修正头尾信息
				//$this->check_jpeg($img, true);
