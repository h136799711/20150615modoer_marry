...
class ms_urlrewriter {
    ...
    var $mapping = array(); //module flag rewrite
    var $list = array();
    var $domain_mod = 0;
    ...
    function __construct() {
        $this->set_mod(_G('cfg','rewrite_mod'));
        $this->set_domain_mod(_G('cfg','city_sldomain'));
        $this->hide_index = _G('cfg','rewrite_hide_index');
        $this->_load_module_flag_mapping();
    ...
    function _load_module_flag_mapping() {
        $mapping = _G('cfg','rewrite_moduleflag');
        if(!$mapping) return;
        $lst = explode("\r\n",$mapping);
        foreach ($lst as $value) {
            list($flag, $newname) = explode('=', trim($value));
            $flag = strtolower(_T($flag));
            $newname = strtolower(_T($newname));
            if(!$flag||!$newname) continue;
            $this->mapping[$flag] = $newname;
        }
    }
