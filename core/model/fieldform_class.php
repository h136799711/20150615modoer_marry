...
class msm_fieldform extends ms_model {
    ...
    function _textarea($val) {
        ...
        if($this->config['html'] == 2) {
            $editor =& $this->loader->lib('editor', NULL, TRUE, $this->ctrid);
    ...
    function _area($val) {
        ...
        $content .= "\t<td $this->style>".$notnull.$this->field['title']."：</td>\r\n";
        $content .= "\t<td>\r\n";
        $content .= "\t<div id=\"area_gcategory\"></div>\r\n";
        $content .= "\t<script type=\"text/javascript\">\r\n";
        $content .= "\t$('#area_gcategory').mselect({\r\n";
        $content .= "\t\t'data':_area,\r\n";
        $content .= "\t\t'name':'{$this->ctrname}',\r\n";
        $aid = $val>0?$val:$this->global['city']['aid'];
        $content .= "\t\t'value':'".(int)$aid."'\r\n";
        $content .= "\t});\r\n";
        $content .= "\t</script>\r\n";
    ...
    function _mappoint($val) {
		$val = $this->is_edit ? $val : (isset($this->config['default']) ? $this->config['default']:'');
