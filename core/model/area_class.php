...
class msm_area extends ms_model {
    ...
    function save($post, $aid = null, $cache=TRUE) {
        $edit = $aid > 0;
        if($edit) {
        ...
        $aid = parent::save($post, $detail?$detail:$aid, $cache);
        if(!$edit) {
    ...
    function import() {
        ...
        if($start > 1) {
            $xmlfile =  _input('xmlfile');
        } else {
            $this->loader->lib('upload_file', NULL, FALSE);
            $UP = new ms_upload_file('area_import_file', 'xml');
            $UP->upload('attachments');
            $xmlfile = $UP->path . DS . $UP->filename;
        }
        if(!file_exists(MUDDER_ROOT . $xmlfile)) {
            redirect('导入文件不存在。');
        }
        $this->loader->helper('mxml');
        if(!$areas = mxml::to_array(MUDDER_ROOT . $xmlfile)) {
            redirect('admincp_area_importfile_invalid');
        }
        $checkexist = _input('city_exists',null, MF_INT);
        $citys = _input('citys', 0, MF_INT_KEY);
        ...
        foreach($areas as $city) {
            ...
            $aid_1 = $this->save($city,null,false);
            if($data_1) foreach($data_1 as $b) {
                if(!$b['name']) continue;
                $data_2 = $b['data'];
                $b['pid'] = $aid_1;
                unset($b['data']);
                $aid_2 = $this->save($b,null,false);
                if($data_2) foreach($data_2 as $s) {
                    if(!$s['name']) continue;
                    $s['pid'] = $aid_2;
                    $aid_3 = $this->save($s,null,false);
                }
            }
            $params=array();
            $params['start'] = $start+1;
            $params['xmlfile'] = $xmlfile;
            $params['city_exists'] = $checkexist;
            $params['citys'] = ++$citys;
            redirect("{$city[name]}[".$start."/".count($areas)."] 导入完成，准备导入下一条数据...",cpurl('modoer','area','import',$params));
        }
        @unlink(MUDDER_ROOT . $xmlfile);
        $this->write_cache();
        return $citys;
    }
