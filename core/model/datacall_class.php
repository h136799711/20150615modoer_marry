...
class msm_datacall extends ms_model {
    ...
    function datacall($key,$is_name = FALSE) {
        ...
        if($this->in_ajax && _input('do')=='datacall') {
            ...
            if($_QUERY[$query['var']] = $this->$fun($callid)) {
                include template('datacall/'.$query['tplname']);
            } else {
                include template('datacall/'.$query['empty_tplname']);
            }
        } else {
            ...
            $restr .= "{include template('datacall/'.{$st}['empty_tplname'])}\r\n";
            $restr .= "{else}\r\n";
            $restr .= "{include template('datacall/'.{$st}['tplname'])}\r\n";
            $restr .= "{/if}\r\n";
            return $restr;
        }
    }
                    ...
                    if(!isset($gbls[$pval])) {
                        ...
                        if(!is_numeric($value)) {
                            if($type == 'field') {
                                ...
                                $value = "'" . str_replace(',', "','", _G('db')->_escape_str($value)) . "'";
                            } else {
                                $value = "'" . _G('db')->_escape_str($value) . "'";
                            }
                        }
                        $gbls[$pval] = $value;
                    }
    ...
    */
    ...
    function delete($callids, $cache=TRUE) {
    ...
    function _import_sql($content) {
        ...
        $table = str_replace('dbpre_', _G('dns','dbpre'), $this->table);
    ...
    function _export_sql() {
        ...
        $row = $this->db->query("SHOW COLUMNS FROM ". str_replace('dbpre_', _G('dns','dbpre'),$this->table));
        $split = '';
