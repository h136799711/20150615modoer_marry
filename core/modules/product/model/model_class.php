        ...
        */
        ...
        if(is_file($ctratefile)) {
            ...
            foreach($create_field as $key => $val) {
                ...
                $F->add($val, false, false); // 建立字段记录
            }
        }
        $F->write_cache($modelid);
        return $modelid;
    }
    ...
    function write_cache($modelid = null, $return = FALSE) {
        ...
        while($value = $row->fetch_array()) {
            ...
            if(!$modelid || $modelid == $value['modelid']) {
                ms_cache::factory()->write('product_model_'.$value['modelid'], $value);
                //write_cache('model_' . $value['modelid'], arrayeval($value), $this->model_flag);
                if($modelid == $value['modelid']) $result = $value;
            }
        }
        $row->free_result();
        ms_cache::factory()->write('product_model', $models);
        //write_cache('model', arrayeval($models), $this->model_flag);
