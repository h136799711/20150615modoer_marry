...
class msm_product extends ms_model {
    ...
    function save($post, $pid = null) {
        ...
        if($post['tag_keyword']) foreach ($post['tag_keyword'] as $key => $value) {
            if(!$value) unset($post['tag_keyword'][$key]);
        }
        if($post['tag_keyword']) {
        ...
        if(!$edit) {
            ...
            //解析错误提示
            if($buyattr_obj->has_error()) redirect($buyattr_obj->error());
        }
    ...
    function check_post($post, $edit = false) {
        ...
        if($post['city_id'] > 0) {
            ...
            if(!$area||!$area['enabled']||$area['pid']>0) redirect('对不起，您选择的城市不存在或已停用。');
        } else {
            $post['city_id'] = 0;
        }
