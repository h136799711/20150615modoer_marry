...
<form method="post" action="<?=cpurl($module, $act, 'save')?>">
    <div class="space">
        ...
        <table class="maintable" border="0" cellspacing="1" cellpadding="1">
            <tr>
                <td class="altbg1" width="45%"><strong>字段名称：</strong>此项在我们设计模板时将通过这个名称取得字段的值，由英文字母，数字和下划线组成，<span class="font_1">此项填写后将无法再次改动</span></td>
                <td width="*">
                    c_<input type="text" class="txtbox4" name="t_field[fieldname]" value="<?=$t_field['fieldname']?>"<?=$disabled?> />
                    <input type="hidden" name="tablename" value="product_data">
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><?=form_bool('t_field[allownull]', isset($t_field['allownull'])?$t_field['allownull']:1)?></td>
            </tr>
            <tr>
            ...
            <tr>
                <td class="altbg1"><strong>是否为后台字段：</strong>不显示在前台会员登记页面，仅供后台用户操作选择。<span class="font_1">添加时必填字段不要设置未后台字段，否则会引起前台用户添加产品时无法提交</span></td>
