...
<div id="body">
<form method="post" name="myform" id="postform" action="<?=cpurl($module, $act, 'add_save') ?>" enctype="multipart/form-data" onsubmit="return validator(this);">
    <div class="space">
        ...
        <table width="95%" cellspacing="0" cellpadding="0" class="maintable">
            ...
            <tr>
				<td  width='*' class='altbg1' align='right'><span class="font_1">*</span>套餐分类：</td>
				<td>
            ...
            <tr>
                ...
                <td width="*"><input type="text" name="name" class="txtbox" class="txtbox" value="<?=$detail['subject'] ?>" validator="{'empty':'N','errmsg':'请填写套餐名称。'}" /></td>
            </tr>
            
            <tr>
                <td class="altbg1" align="right"><span class="font_1">*</span>套餐价格：</td>
                <td><input type="text" name="ori_price" class="txtbox3" value="<?=$detail['price'] ?>" validator="{'empty':'N','errmsg':'未填写套餐价格。'}" /> 元</td>
            </tr>
            ...
            <tr>
                ...
                <td width="*">
                    <input type="text" name="price" id="promote" class="txtbox4" value="<?=$detail['promote'] > 0 ? $detail['promote'] : '' ?>" /> 元，
                    有效期：
                    <input name="start_time" id="promote_start" type="text" class="txtbox4" readonly="true" value="<?if($detail['promote_start']):?><?=date('Y-m-d', $detail['promote_start']) ?><?endif; ?>" onfocus="WdatePicker({doubleCalendar:true,dateFmt:'yyyy-MM-dd'})" />
                     ~ 
            ...
            <tr >
                ...
                <td>
                    <textarea name="tags" style="width:425px;height:50px;"></textarea>
...
function append_product(cont,product){
	...
	cont.append($tr);
	console.log(product);
	var td_list = "<td>{id}</td><td class='name'><img src='{imgsrc}' alt='{name}' />{name}</td><td>{price}</td><td>{stock}</td><td><label><input type='checkbox' name='add_pids[]' value='{id}' />加入套餐</label></td>";
	td_list = td_list.replace(new RegExp("{id}","g"),product.pid).replace(new RegExp("{imgsrc}","g"),product.picture).replace(new RegExp("{name}","g"),product.subject).replace('{price}',product.price).replace('{stock}',product.stock);
	$tr.append($(td_list));
}
