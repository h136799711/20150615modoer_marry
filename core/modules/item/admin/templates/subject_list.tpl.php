    ...
    <div class="space">
        ...
        <table class="maintable" border="0" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <td width="400">
                    <div id="subject_gcategory"></div>
                    <script type="text/javascript">
                        $('#subject_gcategory').mselect({
                            'data':_item_cate,
                            'name':'catid',
                ...
                <td width="*">
					<?if($admin->is_founder):?>
                    <div id="area_gcategory"></div>
                    <script type="text/javascript">
                        _area['data'][-1] = '无地区';
                        array_push(_area['level'][0][0],'-1');
                        $('#area_gcategory').mselect({
                            'data':_area,
                            'name':'aid',
                            'value':'<?=(int)$_GET[aid]?>'
                        });
                    ...
                    <select name="aid" id="aid">
                        ...
                        <?if($admin->check_global()):?>
                        <option value="-1"<?if($_GET['aid']==-1)echo' selected="selected"';?>>全局</option>
                        <?endif;?>
                    </select>
                    <?endif;?>
                </td>
            ...
            <tr>
                ...
                <td width="*"><input type="text" name="sid" class="txtbox4" value="<?=$_GET['sid']?>" /></td>
            </tr>
            <tr>
...
<form method="post" name="myform" action="<?=cpurl($module,$act,'',array('pid'=>$pid))?>">
    <div class="space">
        ...
        <table class="maintable" border="0" cellspacing="0" cellpadding="0" trmouse="Y" >
            <tr class="altbg1">
                ...
                <td width="60">浏览量<?=p_order('pageviews');?></td>
                <td width="35"><center>等级<?=p_order('level');?></center></td>
                <td width="35"><center>点评<?=p_order('reviews');?></center></td>
                <td width="35"><center>图片<?=p_order('pictures');?></center></td>
                <td width="35"><center>留言<?=p_order('guestbooks');?></center></td>
                <td width="100">添加时间<?=p_order('addtime');?></td>
                <td width="60">状态<?=p_order('finer');?></td>
                <td width="60">操作</td>
            </tr>
            <?if($total):?>
    ...
    <center>
        ...
        <?php if(S('item:use_recycle')):?>
        <button type="button" class="btn" onclick="easy_submit('myform','recycle','sids[]')">删除所选(回收站)</button>
        <?php else:?>
