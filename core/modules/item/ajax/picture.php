...
case 'get':
    ...
    if($list) {
        ...
        echo "\t<tr>\t";
        if($_GET['page'] > 1) {
            echo "\t<td class=\"arrow\"><img src=\"".get_imageurl('templates/main/default/img/pic_left.gif')."\" onclick=\"get_pictures($sid,".($_GET['page']-1).");\" /></td>";
        }
        $i = 0;
        while($val = $list->fetch_array()) {
            $i++;
            $url = get_imageurl($val['filename']);
            $thumb = get_imageurl($val['thumb']);
        ...
        if($i == $offset && $total > $start + $offset) {
            echo "\t<td class=\"arrow\"><img src=\"".get_imageurl('templates/main/default/img/pic_right.gif')."\" onclick=\"get_pictures($sid,".($_GET['page']+1).");\" /></td>";
        }
        echo "\t</tr>\t";
        echo "\t</tbody>\t";
        echo '</table>';
