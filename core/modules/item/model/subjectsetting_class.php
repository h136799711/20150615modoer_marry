...
class msm_item_subjectsetting extends ms_model {
    ...
    function check_access($sid) {
        ...
        $this->subject = $this->s->read($sid);
        if(empty($this->subject)) redirect('item_empty');

        $category = $this->s->get_category($this->subject['catid']);
        $catcfg = $category['config'];
        if(!$this->subject['templateid'] && $catcfg['templateid'] > 0) {
            $this->subject['templateid'] = $catcfg['templateid'];
        }
