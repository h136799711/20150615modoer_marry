...
class msm_item_subject extends msm_item_itembase {
    ...
    function getlist($pid, $select, $where, $orderby, $start, $offset, $total = TRUE) {
        ...
        if($atts) {
            ...
            if($num > 1) {
                ...
                $this->db->join($this->table, 's.sid','dbpre_subjectatt', 'st.sid', 'LEFT JOIN');
                if($table) $this->db->join_together('s.sid', $table, 'sf.sid', 'LEFT JOIN');
                $this->db->where('attid', array_pop($attlist));
    ...
    function save($post, $sid = null) {
		...
		'thumb','status','map_lng','map_lat','map_geohash','description');
        ...
        //     if($this->db->from($this->table)->where('cuid',$this->global['user']->uid)->count()>0) {
        //         redirect('对不起，您已经添加了一个主题。');
        //     }
        // }
        ...
        if(!$data['catid']) $data['catid'] = $catid;
        ...
        if(!$edit) {
            ...
            //强制上传封面判断
            if($this->category['config']['subject_add_thumb'] && empty($_FILES['picture']['name'])) {
                redirect('对不起，您没有上传封面图片。');
            }
        }
    ...
    function recycle($ids) {
        ...
        //获取数据，并设置status=-1，同时更新分类数量统计
        while ($value = $row->fetch_array()) {
            if(!$this->in_admin && $this->global['user']->username != $value['owner']) {
                redirect('global_op_access');
            }
            if($value['status'] != '-1') {
                $this->db->from($this->table)->where('sid',$value['sid'])->set('status',-1)->update();
            }
            if($value['status'] > 0) {
                // 分类统计更新
    ...
    function mappoint($sid,$mappoint) {
        ...
        $this->db->set('map_geohash', $map_geohash);
        $this->db->where('sid',$sid);
        $this->db->update();
    }
