...
class msm_item_fielddetail extends msm_fielddetail {
            ...
            if($tags = unserialize($val)) {
                ...
                foreach($tags as $id => $val) {
                    if($this->nourl) {
                        ...
                        $content .= $split . "<a href=\"".url("item/tag/tagname/$val",'',TRUE)."\">$val</a>";
                    }
    ...
    function _video($val) {
        if(_G('in_mobile')) {
            $url = '<a href="'.$val.'" target="_blank">'.lang('item_play_video').'</a>';
        } else {
            $url = '<a href="javascript:;" onclick="open_video($(this).attr(\'data-url\'));" data-url="'._T($val).'">'.lang('item_play_video').'</a>';
        }
