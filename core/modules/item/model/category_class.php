...
class msm_item_category extends ms_model {
    ...
    function write_cache($update_jscache_flag = FALSE) {
        ...
        if($query = $this->db->get()) {
            ...
            $cache->write('item_category', $result);
            $cache->write('item_category_rel', $rel);
            foreach($file as $key => $val) {
                $cache->write('item_category_' . $key, $val);
            }
/*
         ...
         //更新js后，因为浏览器会缓存js文件，所以需要给js做一个更新标识，放在模块配置里面
            if($update_jscache_flag) {
                $this->loader->model('config')->save(array('jscache_flag'=>rand(1,1000)), 'item');
            }
        }
    }
    ...
    function _get_cache($key) {
        ...
        $this->db->order_by(array('level'=>'ASC','listorder'=>'ASC'));
        if($query = $this->db->get()) {
            $cate = $file = $level2 = $level3 = $rel = false;
            while($val = $query->fetch_array()) {
                $rel[$val['catid']] = $val['pid'] . ':' . $val['level'];
                if($val['config']) $val['config'] = unserialize($val['config']);
                if($val['level']=='1') { //城市
        ...
        //缓存
        $cache = ms_cache::factory();
        $cache->write('item_category', $cate);
        if(!$key) $result = $cate;
        $cache->write('item_category_rel', $rel);
        if($key == 'rel') $result = $rel;
        foreach($file as $k => $v) {
            $cache->write('item_category_' . $k, $v);
            if($key == $k) $result = $v;
        }
        //返回值
        return $result;
