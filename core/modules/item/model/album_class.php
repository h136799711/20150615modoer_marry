...
class msm_item_album extends msm_item_itembase {
	...
	//新建一个默认的主题相册
	function create_normal($sid,$name='',$thumb='', $show_error = false) {
		$post = array();
        if(!$sid && $show_error) redirect('对不起，您未指定新建相册的主题ID。'); 
        if(!$name && $show_error) redirect('对不起，您未填写相册名称。');
		$post['sid'] = $sid;
    ...
    function album_total($albumid,$num=1,$thumb='') {
        ...
        if($thumb && is_file(MUDDER_ROOT . $thumb)) {
            ...
            if(!$album) return;
            //图片不存在时替换，存在时不处理 
            if($album['thumb'] && is_file(MUDDER_ROOT . $album['thumb'])) {
                $thumb = '';
            }
        } else {
            $thumb = '';
