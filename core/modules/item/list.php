...
if($catid != $pid) {
    ...
    $attcats = isset($category[$catid]['config']['attcat']) ? $category[$catid]['config']['attcat'] : '';
} else {
...
if($att) {
    foreach($att as $_att_v) {
        ...
        if($_attcatid > 0 && $_attid > 0) {
            ...
            $tagkey = 'att_name_' . $_attcatid;
            $SEO->tags->$tagkey = $_attlist[$_attid]['name'];
        }
    }
}

//seo setting
$SEO->tags->root_area_name = $paid > 0 ? display('modoer:area',"aid/$paid"):($aid>0?display('modoer:area',"aid/$aid"):'');
$SEO->tags->current_area_name = $aid ? display('modoer:area',"aid/$aid"):'';
$SEO->tags->root_category_name = display('item:category',"catid/$pid");
$SEO->tags->current_category_name = display('item:category',"catid/$catid");
$SEO->tags->root_category_keywords = $catcfg['meta_keywords'];
$SEO->tags->root_category_description = $catcfg['meta_description'];
$SEO->tags->page = $_GET['wfpage'] ? $_GET['wfpage'] : $_GET['page'];
$SEO->tags->subject_total = $total;

//解析seo设置赋值
$_HEAD['title'] = $SEO->pares('list_' . $pid)->title;
