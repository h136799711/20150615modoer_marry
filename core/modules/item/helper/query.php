...
class query_item {
    ...
    //获取点评里的标签组
    function tag_groups($params) {
        $tags = $params['tags'];
        if(is_serialized($tags)) {
            $tags = unserialize($tags);
        }
        if(!$tags || !is_array($tags)) return;
        $groups = _G('loader')->variable('taggroup','item');
        if(!$groups) return;
        $result = array();
        foreach ($groups as $key => $value) {
            if($tags[$key]) $result[] = array('taggroup'=>$value,'tags'=>$tags[$key]);
        }
        return $result;
    }
