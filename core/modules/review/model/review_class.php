...
class msm_review extends ms_model {
    ...
    function init_field() {
        parent::add_field('idtype,id,sort1,sort2,sort3,sort4,sort5,sort6,sort7,sort8,price,best,digest,title,content,taggroup,pictures,hide_name,source,voice_file');
        parent::add_field_fun('id,sort1,sort2,sort3,sort4,sort5,sort6,sort7,sort8,price,best,digest,hide_name,source', 'intval');
    ...
    function save($post, $rid = null) {
        ...
        //上传语音文件
        if(isset($post['voice_file'])) {
            $post['voice_file'] = $this->get_voice_file($post['voice_file'],$detail['voice_file']);
        }
        $post['havevoice'] = $post['voice_file'] ? 1 : 0;
        ...
        if(!$is_edit) {
            ...
            if($post['havepic'] && $config['picturecheck']) $post['status'] = 0;
            //告之 控制页面 当前的状态
        ...
        //     if(preg_match("/^sort[1-8]{1}$/", $key)) {
        //         $socrue += $post[$key];
        //         $mun++;
        //     }
        // }
        // $best = $socrue / $mun;
        // if($best>=3.8) $post['best']=2;
        // elseif($best>=2.4) $post['best']=1;
        // else $post['best']=0;
        ...
        if(is_array($post['pictures']) && $post['pictures']) {
            $post['pictures'] = $this->get_upload_pictures($id, $post['pictures'], $detail['pictures']);
        } else {
            $post['pictures'] = '';
        }
    ...
    function check_post($config, $post, $isedit) {
        ...
        //字数限制(如有语音点评，则可以不写点评内容)
        if(!$post['content'] && !$post['havevoice']) {
            redirect('review_content_empty');
        } elseif($post['content']) {
            if(strlen($post['content']) < $this->modcfg['review_min'] || strlen($post['content']) > $this->modcfg['review_max']) {
    ...
    //临时文件夹内图片导入
    function get_upload_pictures($sid, $pictures, $dbpics) {
        $upfiles = array();
        $exfiles = array();
        foreach ($pictures as $key => $value) {
            $value = str_replace(array('/','\\'), '/', $value);
            if(strposex($value, '/temp/')) {
                if(is_file(MUDDER_ROOT . $value)) $upfiles[] = $value;
            } else {
                $exfiles[] = $value;
            }
        }
        $delpicids = $newdbpics = array();
        if($dbpics) $dbpics = unserialize($dbpics);
        if($dbpics) foreach ($dbpics as $picid => $value) {
            //array('thumb'=>$v['thumb'],'picture'=>$v['filename']);
            if(!in_array($value['picture'], $exfiles)) {
                $delpicids[] = $picid;
            } else {
                $newdbpics[$picid] = $value;
            }
        }
        $picids = array();
        $PIC = $this->loader->model('item:picture');
        if($upfiles) foreach ($upfiles as $file) {
            $post = array();
            $post['sid'] = $sid;
            $post['local_picture'] = $file;
            $picids[] = $PIC->save($post, true);
        }
        if($picids) {
            $pic = $PIC->db->from($PIC->table)->where('picid', $picids)->get();
            while ($v = $pic->fetch_array()) {
                $newdbpics[$v['picid']] = array('thumb'=>$v['thumb'],'picture'=>$v['filename']);
            }
        }
        if($delpicids) $PIC->delete($delpicids);
        return $newdbpics ? serialize($newdbpics) : '';
    }

    //获取语音上传文件
    function get_voice_file($sorcuefile, $oldfile) {
        if($oldfile == $sorcuefile) return trim($oldfile);
        if(!$sorcuefile && $oldfile) {
            //vp(MUDDER_ROOT . str_replace('/',DS,$oldfile));
            @unlink(MUDDER_ROOT . str_replace('/',DS,$oldfile));
            return '';
        }

        $realfile = MUDDER_ROOT . $sorcuefile;
        if(!file_exists($realfile)) return '';
        $exts = array('mp3','ogg');
        $file_ext = strtolower(pathinfo($realfile, PATHINFO_EXTENSION));
        if(!in_array($file_ext, $exts)) return '';

        $name = basename($sorcuefile);
        $path = 'uploads' . DS . 'voice';
        ...
        if(!copy($realfile, MUDDER_ROOT . $filename)) {
            redirect('语音文件复制失败！请检查语音文件是否存在或上传目录文件夹权限。');
        }
        @unlink($realfile);
        if($oldfile) @unlink(MUDDER_ROOT.$oldfile);
    ...
    function dec_user_point($uid, $num = 1) {
        if(!$uid) return;
        $this->loader->model('member:point')
            ->update_point($uid, 'add_review', TRUE, $num, FALSE, array('reviews'=>array('set_dec',array($num))));
    }
    ...
    function _delete($where, $update_total = TRUE, $delete_point = FALSE) {
        ...
        while($value=$q->fetch_array()) {
            if(!$this->in_admin && !$this->global['user']->isLogin) {
                redirect('global_op_access');
            }
    ...
    function _feed($rid) {
        ...
        if(!$detail = $this->read($rid)) return;
        if($detail['uid'] > 0 && !$detail['hide_name']) {
    ...
    function _share($passports, $rid) {
        ...
        $images = array();
        if($detail['pictures']) {
            $pictures = @unserialize($detail['pictures']);
            if(is_array($pictures)) foreach ($pictures as $key => $value) {
                $images[] = get_imageurl($value['picture'],true);
            }
        }
        //连接
        $urls = array(
            U("review/detail/id/$rid", true),
        );
        ...
        foreach ($passports as $psname => $enable) {
            ...
            $succeed = $this->loader->lib($psname)->post_text($text, $images, $urls);
        }
    }
}
?>