...
class query_review {
    ...
    function review($params) {
        ...
        $loader =& _G('loader');
        $db =& _G('db');
        if(!$select) $select = 'rid,pcatid,city_id,sid,uid,username,ip,title,content,sort1,sort2,sort3,sort4,sort5,sort6,sort7,sort8,price,best,posttime,hide_name';
        ...
        if($hide_name) $db->where('r.hide_name', 0);
        if(isset($best)) $db->where_more('best', (int)$best);
    ...
    function flowers($params) {
        ...
        $db->where('idtype','review');
        if($rid) $db->where('id', $rid);
