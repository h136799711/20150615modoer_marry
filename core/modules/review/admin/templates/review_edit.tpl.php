...
<form method="post" name="myform" action="<?=cpurl($module,$act,'save')?>">
    ...
    <div class="space">
        ...
        <table class="maintable" border="0" cellspacing="0" cellpadding="0">
            ...
            <?if($detail['pictures']):?>
            <tr>
                <td align="right" valign="top" class="altbg1">上传图片：</td>
                <td>
                    <style>
                        .review_picture_op{float:left;width:105px;margin-right:5px;text-align:center;}
                    </style>
                    <?php $detail['pictures'] = unserialize($detail['pictures']);?>
                    <?php foreach($detail['pictures'] as $picid => $pic):?>
                    <div id="pic_<?=$picid?>_foo" class="review_picture_op">
                        <input type="hidden" name="review[pictures][]" id="pic_<?=$picid?>" value="<?=$picid?>" />
                        <a href="<?=URLROOT.'/'.$pic['picture']?>" target="_blank"><img src="<?=URLROOT.'/'.$pic['thumb']?>" width="100" /></a>
