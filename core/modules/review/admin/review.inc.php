...
switch ($op) {
    ...
    case 'checklist':
        $where = array();
        if(!$admin->is_founder) $where['r.city_id'] = $admin->check_global() ? array(0,$_CITY['aid']) : $_CITY['aid'];
        $where['r.status'] = 0;
        $select = 'r.*';
    ...
    default:
        //if($_GET['dosubmit']) {
            $R->db->from($R->table);
            if(!$admin->is_founder) $R->db->where('city_id', $admin->check_global() ? array(0,$_CITY['aid']) : $_CITY['aid']);
