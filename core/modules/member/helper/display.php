...
class display_member {
    ...
    function point($params) {
		...
		if($point == 'rmb' && array($keyname, array('name','u|n'))) return lang('member_point_rmb');
        if($point == 'point' && array($keyname, array('name','u|n'))) return lang('member_point_level');
        if($keyname == 'u|n') {
            $value= $point_group[$point]['unit'].$point_group[$point]['name'];
        } else {
            $value= $point_group[$point][$keyname];
        }
