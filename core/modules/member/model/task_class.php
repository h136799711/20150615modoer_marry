...
class msm_member_task extends ms_model {
    ...
    //会员放弃任务
    function cancel_mytask($taskid,$always_del=false) {
        ...
        if($mytask['status'] == '1' && !$always_del) {
            redirect('member_task_apply_delete_done');
        }
        $this->db->from($this->mytable);
        $this->db->where('uid', $this->global['user']->uid);
        $this->db->where('taskid', $taskid);
    ...
    function check_progress($taskid) {
        ...
        if(!$tsk) {
            $this->cancel_mytask($taskid, true); //任务主题不存在，放弃删除任务
            return 0;
        }
    ...
    //自动申请后的提示
    function _notice_autoapply() {
        $url = url("member/index/ac/task/op/doing",'',true); 
        $note = lang('member_task_autoapply_notice', $url);
        $N = $this->loader->model('member:notice');
        $N->save($this->global['user']->uid, 'member', 'task', $note);
    }
