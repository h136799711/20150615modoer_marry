...
class msm_member_address extends ms_model {
    ...
    function save($post, $id=null) {
        ...
        if($edit) {
            $detail = $this->read($id);
            if(empty($detail)) $this->add_error('对不起，您编辑的地址信息不存在。');
            if($detail['uid'] != $this->global['user']->uid) $this->add_error('对不起，你编辑的地址信息不属于您。');
        } else {
    ...
    function check_post(&$post, $isedit) {
        if(!$isedit) {
            $address_num_limit = (int)$this->modcfg['address_num_limit'];
            if(!$address_num_limit || $address_num_limit < 1 || $address_num_limit > 20) $address_num_limit = 20;
            if($this->get_count($post['uid']) >= $address_num_limit) {
                $this->add_error('收货地址数量最多只能添加【'.$address_num_limit.'】个。');
            }
        }
    ...
    function delete($id) {
        $detail = $this->read($id);
        if(!$detail) $this->add_error('对不起，你编辑的地址信息不存在。');
        if($detail['uid']!=$this->global['user']->uid) $this->add_error('对不起，你编辑的地址信息不属于您。');
        parent::delete($id);
        return TRUE;
    }
    ...
    {
        return $this->db->from($this->table)->where('uid',$uid)->count();
    }
