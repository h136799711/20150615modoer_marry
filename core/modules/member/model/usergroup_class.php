...
class msm_member_usergroup extends ms_model {
    ...
    //获取全部会员组配置信息
    static function fetch_usergroups()
    {
        if(!self::$usergroup)
        {
            self::$usergroup = _G('loader')->variable('usergroup', 'member');
        }
        return self::$usergroup;
    }

    //通过groupid获取指定的会员组配置信息
    static function fetch_usergroup($groupid)
    {
        $usergroup = self::fetch_usergroups();
        if( ! isset($usergroup[$groupid])) 
        {
            return;            
        }
    ...
    //通过等级积分回去所属会员组ID
    static function point_by_usergroup($point)
    {
        $groupid = 10;
        $usergroup = self::fetch_usergroups();

        if(is_array($usergroup) && $usergroup)
        {
